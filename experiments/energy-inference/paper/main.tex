...
However, two new output-rule variants---\textbf{EGrad} (energy-gradient attention with
additive residual) and \textbf{EDesc} (mixed V=K attention with identity descent)---close
this gap at 400M scale: both V19 EGrad (PPL\,=\,27.8, 49.9\% avg) and V20 EDesc
(PPL\,=\,27.9, 49.5\%) \emph{surpass} the 400M GPT baseline (PPL\,=\,29.8, 48.6\%)
at identical parameter count and FLOPs, marking the first energy-model variants to
outperform GPT at matched compute in our ablation.
...
\subsection{EGrad: energy-gradient output rule with additive residual (V15, V17, V19, V21, V23, V25, V26)}
\label{sec:egrad}
...
For $n_e$-headed blocks with iteration count $L$ (e.g.\ V21: $L=2$, V25: $L=4$,
...
\subsection{EDesc: energy-descent block with mixed V=K attention (V16, V18, V20, V22, V24)}
\label{sec:edesc}
...
%% ============================================================
\subsection{Layer Similarity in EGrad and EDesc}
\label{sec:layer_sim_egrad}
%% ============================================================

The layer similarity analysis above focused on V1 EGPT and V0 GPT.
We now extend it to the EGrad (V15, V19) and EDesc (V16, V20) models,
which differ structurally: they use mixed heads (GPT $V{=}W_V x$ plus energy $V{=}K$ or
energy-gradient heads) with a shared output projection, rather than a pure $V{=}K$ attention.
...
\begin{figure}[ht]
  ...
  \caption{Full $12{\times}12$ inter-layer cosine similarity matrices for attn\_out (top row)
    and ffwd\_out (bottom row) across V0 GPT, V1 EGPT, V15 EGrad, and V16 EDesc.
    V1 EGPT shows the highest attn\_out off-diagonal cosine (0.216) with near-zero ffwd\_out.
    V15 EGrad and V16 EDesc invert this: lower attn\_out similarity (0.107/0.106) but
...
\begin{figure}[ht]
  ...
  \caption{Consecutive-layer cosine similarity for d=768 models.
    ...
    structure); EGrad/EDesc remain below GPT ($\le$0.350 vs GPT $\le$0.403).
    Right: ffwd\_out — EGrad/EDesc show notably higher early-layer FFN similarity
...
\toprule
\textbf{Component} & \textbf{V0 GPT} & \textbf{V1 EGPT} & \textbf{V15 EGrad} & \textbf{V16 EDesc} & \textbf{Random} \\
\midrule
...
\begin{itemize}
  \item \textbf{EGrad/EDesc lose V1 EGPT's shared-attention structure.}
    ...
    EGrad/EDesc mix these heads with standard GPT heads ($V{=}W_V x$), which produce
    diverse outputs.  The blended attention output loses the convergence pattern:
    EGrad (0.107) and EDesc (0.106) are both \emph{below} even V0 GPT (0.121).

  \item \textbf{FFN similarity inverts: EGrad/EDesc show elevated cross-layer alignment.}
    ...
    EGrad/EDesc use additive (EGrad) or gentler subtractive (EDesc) updates that keep
  ...
  \item \textbf{Update coherence is highest for EGrad/EDesc.}
    The full step $\Delta x = h_{\mathrm{out}} - h_{\mathrm{in}}$ shows cross-layer
    cosine of 0.163--0.168 for EGrad/EDesc, far above V0 GPT (0.088) and V1 EGPT (0.026).
    EGrad/EDesc apply more consistent update \emph{directions} across all layers ---
...
\begin{figure}[ht]
  ...
  \caption{Full $24{\times}24$ inter-layer cosine similarity matrices at 400M scale.
    ...
    V19 EGrad and V20 EDesc show lower FFN similarity (0.116/0.126)
...
\begin{figure}[ht]
  ...
  \caption{Consecutive-layer cosine similarity at 400M scale (24 layers).
    ...
    EGrad/EDesc show a mild monotonic rise toward later layers.
    Right: ffwd\_out — V9 GPT shows near-perfect FFN similarity at the first pair (0.928)
    that decays across depth; EGrad/EDesc show uniformly lower values.}
...
\begin{tabular}{lccc}
\toprule
\textbf{Component} & \textbf{V9 GPT} & \textbf{V19 EGrad} & \textbf{V20 EDesc} \\
\midrule
...
are highly redundant, while EGrad/EDesc maintain more diverse FFN representations.
...
\begin{figure}[ht]
  ...
  \caption{Off-diagonal mean attn\_out cosine similarity across all eight models.
    V1 EGPT (0.216) has the highest attention layer similarity; EGrad/EDesc (0.107/0.106)
...
\begin{figure}[h]
  ...
  \caption{Grouped bar chart: per-task accuracy for the output-rule ablation (V0 GPT, V1 EGPT,
    V10 Mixed, V15 EGrad, V16 EDesc), all 144M params / 285 MFLOPs/tok except V0 (162M/321M)
    and V1 (143M/359M).
    Right panels: average accuracy (top) and WikiText PPL (bottom).
    V15 EGrad leads on BoolQ, COPA, SciQ, and average accuracy; V16 EDesc leads on HellaSwag.}
...
\midrule
V15 EGrad & 8E:8G & \textbf{37.0} & \textbf{47.9\%} & 24.8 & 33.5 & 24.9 & \textbf{2.6\%} \\
V16 EDesc & 8E:8G & \textbf{37.0} & 47.4\% & 24.8 & \textbf{33.9} & 23.7 & 2.2\% \\
\bottomrule
\end{tabular}
\end{table}

The 6E+6G (V10) and 8E+8G-with-EGrad/EDesc-rule (V15/V16) variants outperform
higher energy-head ratios.  V14 (10:2) shows no improvement over V13 (8:4) despite
closer approximation to a pure-energy model, confirming that a balanced mix
(or the output-rule change in V15/V16) matters more than the raw proportion of
energy heads.

%% ============================================================
\section{Scaling: EGrad and EDesc at 400M Scale (V19--V24)}
\label{app:scaling}
%% ============================================================

Having established that EGrad (V15) and EDesc (V16) strictly outperform mixed heads (V10)
and match/beat the V0 GPT baseline at 144M/285 MFLOPs/tok, we scale these architectures to
the 400M regime.  Three recurrence schedules are tested, all at $d{=}1024$:
...
\begin{table}[h]
\centering
\caption{400M-scale EGrad/EDesc results vs.\ GPT baselines (30k steps each).
  All runs complete. \textbf{Bold} = best per metric.}
\label{tab:scaling}
\resizebox{\textwidth}{!}{%
...
\begin{figure}[h]
  ...
  \caption{400M-scale model comparison.  Left: WikiText PPL; right: average benchmark accuracy.
    All runs complete.
    V19 EGrad and V20 EDesc both surpass V9 GPT on PPL and average accuracy.
...
\paragraph{Key observations.}
\begin{itemize}
  \item \textbf{Both EGrad and EDesc 24$\times$1 surpass GPT at the same compute budget.}
    V19 EGrad (PPL\,=\,27.8, 49.9\% avg) and V20 EDesc (PPL\,=\,27.9, 49.5\% avg) both
    ...
    surpass the GPT baseline at matched compute.  V19 EGrad edges out V20 EDesc slightly.
...
\begin{figure}[h]
  ...
  \caption{Benchmark accuracy heatmap for all small-scale variants (V0--V18).
    Colour encodes accuracy (green = high, red = low).  V15 EGrad and V16 EDesc
...
\begin{figure}[h]
  ...
  \caption{Scaling with model size.  GPT baselines (V0, V9) form the Pareto frontier.
    EGrad/EDesc variants (V15, V16, V21, V22) sit closest to the GPT frontier among
...
\begin{figure}[h]
  ...
  \caption{Log-scale FLOPs scatter.  On the FLOPs axis recurrent models are no longer
    ...
    (V10/V15/V16) and the 503 MFLOPs cluster (V9/V19/V20).  V15 EGrad and V16 EDesc
...
acquired reliable arithmetic reasoning.  V9 GPT (2.9\%) and V15 EGrad (2.6\%) lead
