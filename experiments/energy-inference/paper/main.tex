...
At the 160M scale, a standard GPT baseline consistently outperforms all EGPT variants on
perplexity, and a 400M EGPT model requires 4.4$\times$ more FLOPs per token to
approximately match the 162M GPT baseline.
However, two new mixed-head variants (V19/V20)---training \emph{intended} as
\textbf{EGrad} (energy-gradient attention) and \textbf{EDesc} (identity descent)
but found to have trained as \textbf{MixedHead} due to a config serialization bug
(see~\S\ref{sec:egrad})---close this gap at 400M scale:
V19 (PPL\,=\,27.8, 49.9\% avg) and V20 (PPL\,=\,27.9, 49.5\%) \emph{surpass}
the 400M GPT baseline (PPL\,=\,29.8, 48.6\%) at identical parameter count and FLOPs,
marking the first mixed-head variants to outperform GPT at matched compute.
True MixH (V15/V16) runs (V27--V36) are currently training.
...
All models share the dolomite-engine \texttt{energy} model type with RoPE positional
encoding, RMSNorm, SwiGLU FFN ($d_{\mathrm{int}} = 2d$ or $3d$), tied input/output
embeddings, and BF16 mixed precision.
Five block variants are ablated; their key differences are the \emph{attention output rule}
and the \emph{block-level update equation}.
We write $h_t \in \mathbb{R}^d$ for the hidden state of token $t$,
$\tilde{h} = \mathrm{RMSNorm}(h)$,
$Q = W_Q \tilde{h}$, $K = W_K \tilde{h}$, $V = W_V \tilde{h}$,
and $A_h = \mathrm{softmax}\!\left(Q_h K_h^\top / \sqrt{d_h}\right) \in \mathbb{R}^{T \times T}$
for the per-head causal attention map.

\subsection{GPT baseline (V0, V9)}

Standard pre-norm decoder transformer:
\begin{align}
  h &\;\leftarrow\; h + W_O\,\bigl[\|_h\, A_h V_h\bigr] + \mathrm{FFN}(\tilde{h}),
\end{align}
where $\|_h$ denotes head concatenation.  No energy structure; purely additive residual.

\subsection{EGPT energy-descent block (V1, V2)}

Each block approximates one gradient-descent step on a learned Hopfield-style energy.
With $V_h = K_h$ the per-head energy is
$E_h = -\sum_{t} \log Z_t + \tfrac{1}{2}\|k_t\|^2$,
whose gradient with respect to $h_t$ via the query role is $A_h K_h$.
The update \emph{subtracts} a learned linear projection of this gradient:
\begin{align}
  g &= W_O\bigl[\|_h\, A_h K_h\bigr] + s_{\mathrm{ff}}\cdot\mathrm{FFN}(\tilde{h}),
  \label{eq:egpt_grad}\\
  h &\;\leftarrow\; h - W\, g,
  \label{eq:egpt_update}
\end{align}
where $W \in \mathbb{R}^{d \times d}$ is an unconstrained linear projection
(\texttt{energy\_proj\_type: unconstrained}) and $s_{\mathrm{ff}}$ is a learned scalar
initialised at 4.  The negative sign implements descent; rotation is captured by the
anti-symmetric part of $W$.

\subsection{Mixed-head EGPT (V10, V11, V13, V14)}

Replaces the $n_h$ pure energy heads with $n_e$ energy heads (V=K) and
$n_g = n_h - n_e$ standard GPT heads (separate $W_V$), combined through a shared
output projection $W_O$:
\begin{align}
  \mathrm{attn\_out} &= W_O\Bigl[\|_{h=1}^{n_e} A_{e,h} K_{e,h}
    \;\Big\|\; \|_{h=1}^{n_g} A_{g,h} V_{g,h}\Bigr],
  \label{eq:mixed_attn}\\
  h &\;\leftarrow\; h - W\,\bigl(\mathrm{attn\_out} + s_{\mathrm{ff}}\cdot\mathrm{FFN}(\tilde{h})\bigr).
  \label{eq:mixed_update}
\end{align}
V10 uses 6E+6G heads; V13 uses 8E+4G; V14 uses 10E+2G.

\subsection{EGrad: energy-gradient output rule with additive residual (V27, V29, V31, V33, V35)}
\label{sec:egrad}

\begin{quote}\textbf{\textcolor{red}{Correction: \textit{Correction: V15/V17/V19/V21/V23 were MixedHead}}}\par\smallskip
A config serialization bug caused \texttt{energy\_grad\_mixed\_head\_attention} and
\texttt{mixed\_head\_energy\_descent} to be silently downgraded to
\texttt{mixed\_head\_attention} during config parsing (\texttt{\_set\_sequence\_mixer\_blocks}
popped the type before constructing \texttt{\_MixedHeadAttentionArgs}, so the default
\texttt{"mixed\_head\_attention"} was used).
All runs V15, V17, V19, V21, V23 were therefore trained as plain \textbf{MixedHead}
(same architecture class as V10), not as the intended EGrad.
Similarly, V16, V18, V20, V22, V24 were trained as plain MixedHead, not EDesc.
The bug has been fixed (\texttt{config/\_\_init\_\_.py}); true MixH (V15/V16) runs
V27--V36 are currently training.  Results for V15--V24 in all tables are retained
with the corrected label ``MixedH'' to distinguish them from the true EGrad (V27+).
\end{quote}

The \textbf{EGrad} variant modifies the energy-head output to be the
\emph{true gradient} of the Hopfield energy with respect to the hidden state $h$
via the query pathway, while keeping a \emph{standard additive residual} (GPT-style block):

\paragraph{Attention.}
For each energy head $h$, compute $Q_{e,h} = W_{Q,h}^e \tilde{h}$ and
$K_{e,h} = W_{K,h}^e \tilde{h}$.
The Hopfield energy (causal, query-role only) is
$E_h(x) = \sum_t \mathrm{LSE}_s(q_{t,h}^\top k_{s,h}/\sqrt{d_h})$,
so $\partial E_h / \partial q_{t,h} = [A_{e,h} K_{e,h}]_t \in \mathbb{R}^{d_h}$.
Pushing this back through $W_{Q,h}^e$ gives the gradient w.r.t.\ $h_t$:
\begin{equation}
  \nabla_{h_t} E_h = {W_{Q,h}^{e}}^\top \bigl[A_{e,h}\, K_{e,h}\bigr]_t.
  \label{eq:egrad_head}
\end{equation}
The $n_g$ GPT heads compute $A_{g,h} V_{g,h}$ as usual, projected through $W_O^g$.
The combined attention output is
\begin{equation}
  \mathrm{attn\_out} = \underbrace{\sum_{h=1}^{n_e} {W_{Q,h}^{e}}^\top
    \bigl[A_{e,h}\, K_{e,h}\bigr]}_{\text{energy gradient}}
  + \underbrace{W_O^g \bigl[\|_h A_{g,h} V_{g,h}\bigr]}_{\text{GPT heads}}.
  \label{eq:egrad_attn}
\end{equation}
Note: unlike V10/EDesc, there is no shared $W_O$ for energy heads; $W_Q^e$ itself serves
as the output projection (no additional parameters from the attention module).

\paragraph{Block update.}
EGrad uses a standard \textbf{additive} pre-norm residual --- not energy descent:
\begin{equation}
  h \;\leftarrow\; h + \mathrm{attn\_out} + \mathrm{FFN}(\tilde{h}).
  \label{eq:egrad_block}
\end{equation}
The energy structure lives entirely in the attention output rule
(Eq.~\ref{eq:egrad_attn}), not in the block-level dynamics.

\paragraph{Recurrent variants.}
For $n_e$-headed blocks with iteration count $L$ (e.g.\ V33: $L=2$, V25: $L=4$,
V26: $L=[2,2,3,4,4,9]$ per layer), the same block weights are applied $L$ times:
\begin{equation}
  h^{(\ell+1)} = \mathrm{EGradBlock}(h^{(\ell)}), \quad \ell = 0, \ldots, L-1.
  \label{eq:egrad_recur}
\end{equation}

\subsection{EDesc: energy-descent block with mixed V=K attention (V28, V30, V32, V34, V36)}
\label{sec:edesc}

The \textbf{EDesc} variant keeps the V=K mixed-head attention of V10
(Eq.~\ref{eq:mixed_attn}) but drops the learned projection $W$ in the block update,
replacing it with the identity:
\begin{equation}
  h \;\leftarrow\; h - \bigl(\mathrm{attn\_out} + s_{\mathrm{ff}}\cdot\mathrm{FFN}(\tilde{h})\bigr).
  \label{eq:edesc_block}
\end{equation}
This is a \emph{pure gradient descent step} (step size 1) on the mixed-head energy,
with no additional projection parameters beyond attention and FFN.
\texttt{energy\_proj\_type: identity} implements Eq.~\ref{eq:edesc_block}.

\paragraph{Comparison with EGrad.}
The two variants differ on both axes simultaneously:

\begin{center}
\begin{tabular}{lll}
\toprule
 & \textbf{Attention output} & \textbf{Block update} \\
\midrule
V10 Mixed & $W_O[A_e K_e \| A_g V_g]$ & subtract $W \cdot g$ \\
EGrad & $\sum_h W_Q^{eT} A_e K_e + W_O^g A_g V_g$ & add (GPT residual) \\
EDesc & $W_O[A_e K_e \| A_g V_g]$ & subtract $g$ (identity) \\
\bottomrule
\end{tabular}
\end{center}

EGrad replaces the V10 output rule with the true energy gradient and switches to additive
residuals, removing the block-level descent.
EDesc keeps the V10 output rule but enforces pure descent by removing the $W$ projection.

Table~\ref{tab:variants} summarises the five original 160M-scale variants;
Tables~\ref{tab:new_variants} and \ref{tab:scaling} cover the extended ablation.
...
%% ============================================================
\subsection{Layer Similarity in Mixed-Head Variants (V15/V16/V19/V20)}
\label{sec:layer_sim_egrad}
%% ============================================================

The layer similarity analysis above focused on V1 EGPT and V0 GPT.
We now extend it to the mixed-head variants V15, V16, V19, V20
(intended as MixH (V15/V16) but trained as MixedHead; see~\S\ref{sec:egrad}).
These models use mixed heads (GPT $V{=}W_V x$ plus energy $V{=}K$ heads)
with a shared output projection, rather than a pure $V{=}K$ attention.
Layer similarity results for the true MixH (V15/V16) (V27--V36) will be added
when those runs complete.

\paragraph{Small scale (d=768, 12$\times$1): attention similarity drops, FFN similarity rises.}

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/layer_sim_cosine_small.pdf}
  \caption{Full $12{\times}12$ inter-layer cosine similarity matrices for attn\_out (top row)
    and ffwd\_out (bottom row) across V0 GPT, V1 EGPT, V15 MixH, and V16 MixH.
    V1 EGPT shows the highest attn\_out off-diagonal cosine (0.216) with near-zero ffwd\_out.
    V15 MixH and V16 MixH invert this: lower attn\_out similarity (0.107/0.106) but
    elevated ffwd\_out similarity (0.157/0.158).}
  \label{fig:layer_sim_cosine_small}
\end{figure}

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/consecutive_sim_small.pdf}
  \caption{Consecutive-layer cosine similarity for d=768 models.
    Left: attn\_out — V1 EGPT rises monotonically to 0.531 at layers 8--9 (peak of shared-energy
    structure); MixH (V15/V16) remain below GPT ($\le$0.350 vs GPT $\le$0.403).
    Right: ffwd\_out — MixH (V15/V16) show notably higher early-layer FFN similarity
    (consecutive max 0.634/0.648 vs V0 GPT 0.480 and V1 EGPT $\approx$0).
    The random-init baseline is near zero throughout.}
  \label{fig:consecutive_sim_small}
\end{figure}

Table~\ref{tab:layer_sim_egrad} summarises the off-diagonal mean cosine similarity.

\begin{table}[h]
\centering
\caption{Off-diagonal mean cosine similarity across models.  Random = untrained V1 EGPT.
  Highest in each row \textbf{bold}.}
\label{tab:layer_sim_egrad}
\small
\begin{tabular}{lcccccc}
\toprule
\textbf{Component} & \textbf{V0 GPT} & \textbf{V1 EGPT} & \textbf{V15 MixH} & \textbf{V16 MixH} & \textbf{Random} \\
\midrule
attn\_out  & 0.121 & \textbf{0.216} & 0.107 & 0.106 & 0.000 \\
ffwd\_out  & 0.080 & 0.000 & \textbf{0.157} & 0.158 & 0.002 \\
update ($\Delta x$) & 0.088 & 0.026 & \textbf{0.168} & 0.163 & 0.001 \\
\bottomrule
\end{tabular}
\end{table}

\begin{itemize}
  \item \textbf{MixH (V15/V16) lose V1 EGPT's shared-attention structure.}
    V1 EGPT's high attn\_out cosine (0.216) arose because the $V{=}K$ constraint makes
    every attention head output a weighted sum of keys, which converges to a shared
    direction in the later layers.
    MixH (V15/V16) mix these heads with standard GPT heads ($V{=}W_V x$), which produce
    diverse outputs.  The blended attention output loses the convergence pattern:
    MixH (0.107) and MixH (0.106) are both \emph{below} even V0 GPT (0.121).

  \item \textbf{FFN similarity inverts: MixH (V15/V16) show elevated cross-layer alignment.}
    V1 EGPT's subtractive update causes the residual stream $h$ to vary drastically
    between adjacent layers, so each FFN sees a different input and produces orthogonal
    outputs (cosine $\approx 0$).
    MixH (V15/V16) use additive (MixH-A) or gentler subtractive (MixH-B) updates that keep
    the residual stream more stable across layers.
    Consequently, the FFN sees more similar inputs at adjacent layers, producing
    cross-layer FFN similarity of 0.157--0.158 --- twice V0 GPT's 0.080.
    The consecutive FFN peak at layers 0--1 (0.634/0.648) suggests early-layer FFNs
    are particularly redundant.

  \item \textbf{Update coherence is highest for MixH (V15/V16).}
    The full step $\Delta x = h_{\mathrm{out}} - h_{\mathrm{in}}$ shows cross-layer
    cosine of 0.163--0.168 for MixH (V15/V16), far above V0 GPT (0.088) and V1 EGPT (0.026).
    MixH (V15/V16) apply more consistent update \emph{directions} across all layers ---
    a form of global coherence that may contribute to their improved benchmark accuracy.
\end{itemize}

\paragraph{400M scale (d=1024, 24$\times$1): GPT dominates FFN similarity.}

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/layer_sim_cosine_400m.pdf}
  \caption{Full $24{\times}24$ inter-layer cosine similarity matrices at 400M scale.
    V9 GPT shows extremely high FFN similarity (off-diag 0.267, consecutive max 0.928 at pair 0--1).
    V19 MixH and V20 MixH show lower FFN similarity (0.116/0.126)
    and similar attention similarity to V9 GPT ($\approx 0.094$).}
  \label{fig:layer_sim_cosine_400m}
\end{figure}

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/consecutive_sim_400m.pdf}
  \caption{Consecutive-layer cosine similarity at 400M scale (24 layers).
    Left: attn\_out — all three models stay below 0.50 with no clear trend;
    MixH (V15/V16) show a mild monotonic rise toward later layers.
    Right: ffwd\_out — V9 GPT shows near-perfect FFN similarity at the first pair (0.928)
    that decays across depth; MixH (V15/V16) show uniformly lower values.}
  \label{fig:consecutive_sim_400m}
\end{figure}

\begin{table}[h]
\centering
\caption{Off-diagonal mean cosine similarity at 400M scale (d=1024, 24$\times$1).}
\label{tab:layer_sim_400m}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Component} & \textbf{V9 GPT} & \textbf{V19 MixH} & \textbf{V20 MixH} \\
\midrule
attn\_out  & 0.093 & 0.094 & \textbf{0.095} \\
ffwd\_out  & \textbf{0.267} & 0.116 & 0.126 \\
update ($\Delta x$) & \textbf{0.235} & 0.106 & 0.119 \\
\bottomrule
\end{tabular}
\end{table}

At 400M scale, EGrad and EDesc show near-identical attention similarity to V9 GPT
(0.094/0.095 vs 0.093).
Strikingly, V9 GPT now leads on FFN similarity (0.267) --- nearly $2.3\times$ higher
than V19/V20 --- with consecutive FFN cosine of 0.928 at the first layer pair,
decaying across depth.
This suggests that at wider models ($d{=}1024$, 24 layers), GPT's FFN early layers
are highly redundant, while MixH (V15/V16) maintain more diverse FFN representations.
The performance advantage of V19/V20 over V9 GPT therefore does not come from
greater inter-layer representation sharing, but from the structure of the attention
output rule itself.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.75\textwidth]{../results/multi-block-ablation/plots/layer_sim_summary_egrad_edesc.pdf}
  \caption{Off-diagonal mean attn\_out cosine similarity across all eight models.
    V1 EGPT (0.216) has the highest attention layer similarity; MixH (V15/V16) (0.107/0.106)
    are below even V0 GPT (0.121).  At 400M scale (right group), all three models are
    near-identical on attn ($\approx$0.09).}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/mixed_iso_bar.pdf}
  \caption{Grouped bar chart: per-task accuracy for all six iso-family models.
    Right panels show average accuracy (top) and WikiText PPL (bottom) per model.
    V10 Mixed 12$\times$1 and V11 Mixed 6$\times$2 match or exceed their GPT counterparts
    on most individual tasks; V2 EGPT 6$\times$2 trails on accuracy but leads on BoolQ and COPA.}
  \label{fig:mixed_iso_bar}
\end{figure}
...
\subsection{Mixed-Head Variants V15/V16 (intended MixH (V15/V16); pending true runs V27/V28)}
...
improve above the GPT baseline.  We next ask whether changing the output rule can close
this gap.  Two variants were intended:

\begin{itemize}
  \item \textbf{V15 (intended EGrad)}: Energy heads to output the true energy gradient
    $W_Q^\top \sigma(QK^\top\!/\!\sqrt{d_h})\,K$; due to the config bug (\S\ref{sec:egrad})
    both V15 and V16 trained as plain \textbf{MixedHead} (identical architecture to V10).
    True EGrad run: \textbf{V27} (currently training).
  \item \textbf{V16 (intended EDesc)}: Mixed-head output used as gradient direction
    $h \leftarrow h - \mathrm{attn\_out}$; also trained as MixedHead.
    True EDesc run: \textbf{V28} (currently training).
\end{itemize}

\begin{table}[h]
\centering
\caption{Mixed-head ablation results (all 12$\times$1, $d{=}768$, 30k steps).
  V15/V16 intended as MixH (V15/V16) but trained as MixedHead (config bug; see~\S\ref{sec:egrad}).
  V13/V14 eval pending.  \textbf{Bold} = best per metric.}
\label{tab:new_variants}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrrrrrrrrrrr}
\toprule
\textbf{Metric} & \textbf{V0 GPT} & \textbf{V1 EGPT} & \textbf{V10 Mix} & \textbf{V15 MixH$^\dagger$} & \textbf{V16 MixH$^\dagger$} \\
...
\begin{figure}[h]
  ...
  \caption{Absolute accuracy heatmap for the output-rule ablation.  V15 (MixH) and
    V16 (MixH) are both iso-param and iso-FLOP to V10 Mixed (144M / 285 MFLOPs/tok).}
...
\begin{figure}[h]
  ...
  \caption{Accuracy delta vs.\ V0 GPT baseline.  V15 MixH shows broad improvements
    across tasks; V16 MixH recovers GSM8K parity with V0.}
...
\begin{figure}[h]
  ...
  \caption{Perplexity vs.\ accuracy: V15 MixH achieves the best PPL and best avg accuracy
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/new_variants_bar.pdf}
  \caption{Grouped bar chart: per-task accuracy for the output-rule ablation (V0 GPT, V1 EGPT,
    V10 Mixed, V15 MixH, V16 MixH), all 144M params / 285 MFLOPs/tok except V0 (162M/321M)
    and V1 (143M/359M).
    Right panels: average accuracy (top) and WikiText PPL (bottom).
    V15 MixH leads on BoolQ, COPA, SciQ, and average accuracy; V16 MixH leads on HellaSwag.}
  \label{fig:new_variants_bar}
\end{figure}

\paragraph{Key findings.}
\begin{enumerate}
  \item \textbf{V15 MixH strictly dominates V10 Mixed and V0 GPT.}
  ...
  \item \textbf{V16 MixH matches V0 on GSM8K and achieves competitive accuracy.}
...
\subsection{Head-Ratio Ablation (V13, V14)}

V13 and V14 vary the energy-to-GPT head ratio within the same 12$\times$1,
$d{=}768$ family as V10, to test whether a specific ratio is optimal:

\begin{itemize}
  \item \textbf{V13 (8E+4G, 143M)}: 8 energy heads, 4 GPT heads, intermediate=1536.
  \item \textbf{V14 (10E+2G, 142M)}: 10 energy heads, 2 GPT heads, intermediate=1536.
\end{itemize}

Both use the same training setup (30k steps, lr=2e-3) and are iso-param/iso-FLOP
to V10 (6E+6G) at 142 MFLOPs/token.

\begin{table}[h]
\centering
\caption{Head-ratio ablation (12$\times$1, $d{=}768$, 142 MFLOPs/tok, 30k steps).}
\label{tab:head_ratio}
\begin{tabular}{lrrrrrrr}
\toprule
\textbf{Model} & \textbf{E:G ratio} & \textbf{PPL}$\downarrow$ & \textbf{Avg}$\uparrow$
  & \textbf{ARC-C} & \textbf{HellaS} & \textbf{MMLU} & \textbf{GSM8K} \\
\midrule
V10 Mixed & 6:6 & \textbf{37.0} & 47.2\% & \textbf{25.5} & 33.4 & \textbf{25.7} & 1.5\% \\
V13       & 8:4 & 37.9 & 45.6\% & 24.4 & \textbf{33.3} & 24.6 & 1.7\% \\
V14       & 10:2 & 39.5 & 46.1\% & 25.7 & 32.6 & 25.8 & 1.6\% \\
\midrule
V15 MixH$^\dagger$ & 8E:8G & \textbf{37.0} & \textbf{47.9\%} & 24.8 & 33.5 & 24.9 & \textbf{2.6\%} \\
V16 MixH$^\dagger$ & 8E:8G & \textbf{37.0} & 47.4\% & 24.8 & \textbf{33.9} & 23.7 & 2.2\% \\
\bottomrule
\end{tabular}
{\footnotesize $^\dagger$ Intended as MixH (V15/V16); trained as plain MixedHead due to config bug. True EGrad (V27) and EDesc (V28) currently training.}
\end{table}

The 6E+6G (V10) and 8E+8G (V15/V16) variants outperform higher energy-head ratios.
V14 (10:2) shows no improvement over V13 (8:4) despite closer approximation to a
pure-energy model, confirming that the head balance matters more than proportion.
Note: V15/V16 showing marginal improvement over V10 likely reflects seed variance
(identical architecture); definitive comparison awaits V27/V28.

%% ============================================================
\section{Scaling: Mixed-Head Variants at 400M Scale (V19--V24; true MixH (V15/V16) V31--V36 pending)}
\label{app:scaling}
%% ============================================================

Having established that mixed-head variants V15/V16 improve over V10 (though the
marginal gap may be seed variance), we scale the same architecture family to the 400M
regime.  Note: V19--V24 were \emph{intended} as MixH (V15/V16) but trained as
\textbf{plain MixedHead} due to the same config bug (\S\ref{sec:egrad}).  Their
results are valid but reflect MixedHead, not the energy-gradient or energy-descent
output rules.  True MixH (V15/V16) runs V31--V36 are currently training.  Three recurrence schedules are tested, all at $d{=}1024$:

\begin{itemize}
  \item \textbf{V19/V20 (24$\times$1, 342M)}: 24 distinct blocks, 1 iteration each --- exact
    recurrence-free analogue of V9 GPT.  Iso-param (341.9M) and iso-compute (503 MFLOPs/tok)
    to V9.
  \item \textbf{V21/V22 (6$\times$2, 162M)}: 6 unique blocks, 2 iterations each.  Unique
    params $\approx$ half of V19/V20 (162.5M), same 12 effective layer passes, same 252 MFLOPs/tok.
  \item \textbf{V23/V24 (12$\times$2, 228M)}: 12 unique blocks, 2 iterations each.
    Effective depth = 24 passes, same 503 MFLOPs/tok as V9/V19/V20, but with weight sharing
    across the two passes. \emph{(Pending.)}
\end{itemize}

V19/V20 completed training (30k steps, 7.86B tokens).  V21/V22 also completed.
V23/V24 are currently training.

\begin{table}[h]
\centering
\caption{400M-scale mixed-head results vs.\ GPT baselines (30k steps each).
  V19--V24 intended as MixH (V15/V16) but trained as MixedHead (config bug, see~\S\ref{sec:egrad}).
  True MixH (V15/V16) (V31--V36) currently training. \textbf{Bold} = best per metric.}
\label{tab:scaling}
\resizebox{\textwidth}{!}{%
...
\midrule
V31 EGrad 24$\times$1  & EGrad    & 342M & 503 & \multicolumn{5}{c}{\emph{training}} \\
V32 EDesc 24$\times$1  & EDesc    & 342M & 503 & \multicolumn{5}{c}{\emph{training}} \\
V33 EGrad 6$\times$2   & EGrad    & 162M & 252 & \multicolumn{5}{c}{\emph{training}} \\
V34 EDesc 6$\times$2   & EDesc    & 162M & 252 & \multicolumn{5}{c}{\emph{training}} \\
V35 EGrad 12$\times$2  & EGrad    & 228M & 503 & \multicolumn{5}{c}{\emph{training}} \\
V36 EDesc 12$\times$2  & EDesc    & 228M & 503 & \multicolumn{5}{c}{\emph{training}} \\
\bottomrule
\end{tabular}
{\footnotesize $^\dagger$ Trained as MixedHead (config bug); MixH (V15/V16) output rules not active.}}
\end{table}

\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/bar_400m_models.pdf}
  \caption{400M-scale model comparison.  Left: WikiText PPL; right: average benchmark accuracy.
    All runs complete.
    V19 MixH and V20 MixH both surpass V9 GPT on PPL and average accuracy.
    V21/V22 (6$\times$2, 162M) achieve comparable accuracy to V1\_400m at half the parameters.}
  \label{fig:bar_400m}
\end{figure}

\paragraph{Key observations.}
\begin{itemize}
  \item \textbf{Both MixH 24$\times$1 surpass GPT at the same compute budget.}
    V19 MixH (PPL\,=\,27.8, 49.9\% avg) and V20 MixH (PPL\,=\,27.9, 49.5\% avg) both
    outperform V9 GPT (29.8 PPL, 48.6\%) and V1 EGPT (38.6 PPL, 47.0\%) at identical
    or lower FLOPs.  These are the first energy-model variants in our ablation to
    surpass the GPT baseline at matched compute.  V19 MixH edges out V20 MixH slightly.
  \item \textbf{Recurrent 6$\times$2 is parameter-efficient.}
    V21/V22 (162M, 252 MFLOPs/tok) achieve PPL $\approx$ 37, matching the best 144M models
    at twice the FLOPs, and approach V9's accuracy at one-third the compute.
  \item \textbf{Recurrent 12$\times$2 bridges the gap at matched FLOPs.}
    V23/V24 (228M, 503 MFLOPs/tok) achieve PPL $\approx$ 31.7 and $\sim$48\% avg accuracy ---
    substantially better than V21/V22 at the same compute, but still below V19/V20 (342M, no weight sharing).
    Weight-sharing at this scale costs $\sim$4 PPL points vs.\ the non-recurrent counterpart.
\end{itemize}

%% ============================================================
\section{Cross-Scale Scatter Plots}
\label{app:scatter}
%% ============================================================

Figures~\ref{fig:scatter_ppl_params}--\ref{fig:scatter_acc_flops} collect all completed
models (V0--V24) on unified axes to reveal scaling trends across architectures.
...
\begin{figure}[h]
  ...
  \caption{Benchmark accuracy heatmap for all small-scale variants (V0--V18).
    Colour encodes accuracy (green = high, red = low).  V15 MixH and V16 MixH
    occupy the top rows alongside V0 GPT, confirming their consistent improvements.}
...
\begin{figure}[h]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/scatter_ppl_vs_params.pdf}
    \caption{WikiText PPL vs.\ parameter count.}
    \label{fig:scatter_ppl_params}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/scatter_acc_vs_params.pdf}
    \caption{Average accuracy vs.\ parameter count.}
    \label{fig:scatter_acc_params}
  \end{subfigure}
  \caption{Scaling with model size.  GPT baselines (V0, V9) form the Pareto frontier.
    MixH (V15/V16) variants (V15, V16, V21, V22) sit closest to the GPT frontier among
    energy-based models.  Helmholtz variants (V6--V8) are clear outliers.}
...
\begin{figure}[h]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/scatter_acc_vs_flops.pdf}
    \caption{Average accuracy vs.\ FLOPs/token.}
    \label{fig:scatter_acc_flops}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/scatter_ppl_vs_flops.pdf}
    \caption{WikiText PPL vs.\ FLOPs/token.}
    \label{fig:scatter_ppl_flops}
  \end{subfigure}
  \caption{Compute efficiency (FLOPs/token = $2 \times \sum_{\ell} \text{iter}_\ell \times
    (4d^2 + 3d \cdot \text{int})$, forward pass only).
    V15/V16 achieve the best accuracy-per-FLOP among all energy models at 142 MFLOPs/tok.
    V9 GPT (503 MFLOPs/tok) remains the strongest single model.
    Recurrent variants (6$\times$2, 12$\times$2) sit between the 12$\times$1 and 24$\times$1
    clusters, offering parameter sharing without proportional compute savings.}
  \label{fig:scatter_flops}
\end{figure}

\paragraph{Cleaner view: log scale, no Helmholtz, recurrent models as squares.}
Figures~\ref{fig:scatter_params_v2} and \ref{fig:scatter_flops_v2} repeat the scatter
analysis with three changes: (1) log $x$-axis to separate the dense clusters at 118--165M
and 340--355M params; (2) Helmholtz variants (V6--V8) excluded as they are far off the
efficiency frontier and clutter the axes; (3) recurrent models ($6\!\times\!2$,
$12\!\times\!2$) shown as squares to distinguish weight-sharing from non-recurrent
architectures.  Labels are placed with \texttt{adjustText} to minimise overlap.

\begin{figure}[h]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/scatter_ppl_vs_params_v2.pdf}
    \caption{PPL vs.\ params (log $x$).}
    \label{fig:scatter_ppl_params_v2}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    ...
    \label{fig:scatter_acc_params_v2}
  \end{subfigure}
  \caption{Log-scale parameter scatter (Helmholtz excluded, squares = recurrent).
    Recurrent 6$\times$2 models (V11, V17, V18, V21, V22) cluster at $\sim$118--163M
    unique params but achieve effective depth equivalent to their 12$\times$1 or 24$\times$1
    counterparts.}
  \label{fig:scatter_params_v2}
\end{figure}
...
\begin{figure}[h]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Avg accuracy vs.\ FLOPs (log $x$).}
    \label{fig:scatter_acc_flops_v2}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/scatter_ppl_vs_flops_v2.pdf}
    \caption{PPL vs.\ FLOPs (log $x$).}
    \label{fig:scatter_ppl_flops_v2}
  \end{subfigure}
  \caption{Log-scale FLOPs scatter.  On the FLOPs axis recurrent models are no longer
    outliers: V21/V22 (6$\times$2, 252 MFLOPs) sit between the 142 MFLOPs cluster
    (V10/V15/V16) and the 503 MFLOPs cluster (V9/V19/V20).  V15 MixH and V16 MixH
    are the best-performing models per FLOP below 200 MFLOPs/tok.}
  \label{fig:scatter_flops_v2}
\end{figure}

%% ============================================================
\section{GSM8K Mathematical Reasoning Scatter}
\label{app:gsm8k_scatter}
%% ============================================================

Figures~\ref{fig:scatter_gsm8k_params} and \ref{fig:scatter_gsm8k_flops} show GSM8K
greedy exact-match accuracy (flexible extract) against model size and compute for all
completed variants (excluding Helmholtz).  Scores are uniformly low ($<$3\%) across all
architectures at this scale, confirming that 30k-step 100--350M models have not yet
acquired reliable arithmetic reasoning.  V9 GPT (2.9\%) and V15 MixH (2.6\%) lead
among the 144M and 350M clusters respectively; no consistent advantage for energy-based
architectures is evident on this task.

\begin{figure}[h]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/scatter_gsm8k_vs_params.pdf}
    \caption{GSM8K vs.\ parameters (log $x$).}
    \label{fig:scatter_gsm8k_params}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../results/multi-block-ablation/plots/scatter_gsm8k_vs_flops.pdf}
    \caption{GSM8K vs.\ FLOPs/token (log $x$).}
    \label{fig:scatter_gsm8k_flops}
  \end{subfigure}
  \caption{GSM8K exact-match accuracy across all model variants.  All models score below
    3\%; no architecture consistently outperforms others on mathematical reasoning at this
    training scale.}
  \label{fig:scatter_gsm8k}
\end{figure}
