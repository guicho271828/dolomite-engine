...
V27--V36 (currently training).  At 144M parameters / 285 MFLOPs/token, the MixedHead
variant (V15) achieves 47.9\% average accuracy and 36.96 PPL vs.\ GPT's 47.2\% / 38.31 PPL.
At 400M / 503 MFLOPs/token, V19 MixH achieves 49.9\% avg and 27.8 PPL, beating the
matched GPT baseline (48.6\% / 29.8 PPL).
...
are \textbf{V28--V36} (all currently training).
...
\subsection{Mixed-Head Output-Rule Variants V15/V16 (EGrad/EDesc pending V27/V28)}
...
\textbf{Actual (both):} trained as plain MixedHead due to the config bug above.  True EGrad run: V27.  True EDesc run: V28.

Both V15 and V16 are iso-param (144M) and iso-FLOP (285 MFLOPs/tok) to V10 Mixed;
the performance difference vs.\ V10 may be seed variance (architectures are identical).

\begin{table}[h]
\centering
\caption{Output-rule ablation (all 12$\times$1, $d{=}768$, 30k steps, 144M/285 MFLOPs/tok
  except V0/V1).  V15/V16 trained as MixedHead (config bug).
  True EGrad (V27) and EDesc (V28) pending.  \textbf{Bold} = best per metric.}
...
\midrule
V27 EGrad    & \multicolumn{6}{c}{\emph{training}} \\
V28 EDesc    & \multicolumn{6}{c}{\emph{training}} \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  \item \textbf{Mixed-head (V15) shows marginal improvement over V10 and V0 GPT.}
    ...
    all at 144M params / 285 MFLOPs/token.
    Since V15 and V10 are architecturally identical, the gap likely reflects seed variance.
    True EGrad (V27) will establish whether the energy-gradient output rule provides
    additional benefit beyond the MixedHead architecture.

  \item \textbf{V16 MixH recovers the GSM8K regression relative to V10.}
    V16 achieves 47.4\% accuracy and 2.20\% GSM8K, matching V0 GPT on the latter.
    The regression in V10 Mixed (1.52\% GSM8K) is partially recovered, though
    attribution awaits true EDesc (V28).

  \item \textbf{The output rule matters more than the head ratio (to be confirmed).}
    V13 (8:4 ratio) and V14 (10:2) both underperform V10 (6:6) with the $V{=}K$ rule.
    Whether the EGrad/EDesc output rule provides additional gain over 8E+8G MixedHead
    will be determined by V27/V28.
\end{enumerate}
...
\begin{table}[h]
\centering
\caption{400M-scale results (30k steps, 7.86B tokens each). V19--V24 trained as MixedHead (config bug); true EGrad/EDesc (V31--V36) pending.
...
\begin{itemize}
  \item \textbf{MixedHead 24$\times$1 (V19/V20) surpasses GPT at the same compute.}
    ...
    The advantage is $-2.0$ PPL and $+1.3$ pp accuracy for MixH. Whether true EGrad (V31) provides additional benefit is pending.
