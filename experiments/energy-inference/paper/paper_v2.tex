...
At 400M / 503 MFLOPs/token, true EGrad V31 achieves 50.7\% avg and 27.9 PPL,
outperforming matched GPT (48.6\% / 29.8 PPL) and slightly exceeding MixH$^\dagger$ V19 (49.9\% / 27.8 PPL) ---
...
\begin{figure}[h]
  ...
  \caption{Headline results --- PPL and average accuracy vs.\ parameter count (log scale).
    ...
    MixH$^\dagger$ (grey; mislabeled config-bug runs) excluded from this view ---
...
The $V{=}K$ constraint makes the attention output a weighted sum of keys $[A_h K_h]_t$,
which lies on the key manifold in \emph{head space} ($\mathbb{R}^{d_h}$).
This is \textbf{not} the true gradient of the energy $E_h(h_t)=-\log\sum_s A_{h,ts}$
with respect to $h_t$ in hidden space ($\mathbb{R}^D$).
The true gradient requires an additional $W_{Q,h}^\top$ projection back to $\mathbb{R}^D$:
$\nabla_{h_t} E_h = W_{Q,h}^\top [A_h K_h]_t$ (derived in \S\ref{sec:output_rule}).
EGPT's learned projection matrix $W$ partially compensates, but it is an unconstrained
$d{\times}d$ matrix rather than the analytically correct $W_{Q,h}^\top$.
...
are \textbf{V28--V36}.  All V27--V36 runs are now complete (30k steps each).
...
\textbf{EDesc(attn)} (V28 12$\times$1, V30 6$\times$2, V34 6$\times$2 d=1024) results are now available.
...
\begin{figure}[h]
  ...
  \caption{EGrad(attn)/EDesc(attn) (V27--V36) vs.\ baselines: accuracy vs.\ parameters
    (MixH$^\dagger$ mislabeled runs excluded; see Appendix~\ref{sec:appendix_scatter} for comparison).
    EGrad (red) and EDesc (green) cluster tightly with Mixed (purple), confirming
...
\begin{table}[h]
\centering
\caption{400M-scale results (30k steps, 7.86B tokens each). V19--V24 trained as MixedHead (config bug); V31--V36 are true EGrad/EDesc.
  \textbf{Bold} = best per metric.}
\label{tab:scaling}
\resizebox{\textwidth}{!}{%
...
\midrule
V19 MixH$^\dagger$ 24$\times$1  & MixHead & 342M & 503 & \textbf{27.8} & 49.9\% & \textbf{27.7} & \textbf{40.5} & 2.1\% \\
...
V35 EGrad 12$\times$2 1024      & EGrad   & 228M & 503 & 32.0 & 48.8\% & 26.3 & 37.3 & 1.6\% \\
V36 EDesc 12$\times$2 1024      & EDesc   & 228M & 503 & 32.3 & 48.9\% & 26.1 & 37.5 & 1.7\% \\
\bottomrule
\end{tabular}}
{\footnotesize $^\dagger$ Intended as EGrad/EDesc; trained as plain MixedHead (architecture identical to Mixed variants).}
\end{table}
...
\begin{figure}[h]
  ...
  \caption{True EGrad/EDesc (V27--V36) on the efficiency frontier.
    ...
    At 400M / 503 MFLOPs, V31 EGrad (50.7\%) slightly exceeds V19 MixH$^\dagger$ (49.9\%).}
...
\begin{itemize}
  \item \textbf{MixedHead 24$\times$1 (V19/V20) surpasses GPT at the same compute.}
    ...
    True EGrad V31 (50.7\% avg, PPL=27.9) slightly exceeds V19 MixH$^\dagger$ (49.9\%, PPL=27.8),
...
They are MixH$^\dagger$ variants testing different recurrence schedules:
\begin{itemize}
  \item \textbf{V25 MixH$^\dagger$ 6$\times$4} (4 uniform iterations per block, 162M unique params,
    503 MFLOPs/tok): tests whether more recurrence with the same per-layer FLOPs as V23/V24 improves accuracy.
  \item \textbf{V26 MixH$^\dagger$ 6$\times$ramp} ([2,2,3,4,4,9] iterations, 503 MFLOPs/tok):
...
\begin{table}[h]
\centering
\caption{Recurrence-schedule ablation (MixH$^\dagger$, 162M unique params, 503 MFLOPs/tok, 30k steps).
...
V24 MixH$^\dagger$ 12$\times$2 & MixHead & 228M & 503 & 31.7 & 48.5\% & 25.0 & 37.3 & 1.8\% \\
\midrule
V25 MixH$^\dagger$ 6$\times$4  & MixHead & 162M & 503 & \emph{pending} & \emph{pending} & -- & -- & -- \\
V26 MixH$^\dagger$ 6$\times$ramp & MixHead & 162M & 503 & \emph{pending} & \emph{pending} & -- & -- & -- \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{enumerate}
  ...
  \item At 400M / 503 MFLOPs/tok, true EGrad (V31) achieves 50.7\% avg / 27.9 PPL,
    ...
    and narrowly exceeding MixH$^\dagger$ V19 (49.9\% / 27.8 PPL) --- the first consistent energy-model win over GPT.
...
(stored during the forward pass to avoid FSDP weight gathering),
$\lambda$ is the regularisation coefficient, and $\mathcal{P}$ contains the \emph{sparse}
offset pairs $(i,i{+}1)$, $(i,i{+}2)$, $(i,i{+}4)$ to also encourage medium-range similarity.
To reduce overhead, the regularisation is only applied every $k$ steps
($k{=}10$ in all runs, i.e.\ 10\% of steps contribute the regularisation gradient).

The regularisation is applied in \texttt{ModelWrapperForPretraining.get\_loss()} via the
helper \texttt{\_compute\_attn\_cos\_reg()}, which walks \texttt{model.model.transformer.h},
reads the stored \texttt{block.\_attn\_out}, and computes the penalty.  This is purely
activation-based and introduces no new learnable parameters.

\paragraph{Runs.}
\begin{itemize}
  \item \textbf{V39} (160M, $\lambda{=}0.01$, $k{=}10$): EGPT 12$\times$1, $d{=}768$, lr=2e-3.
...
\paragraph{Results.}
\emph{V39 and V40 are currently training (jobs 37576, 37577 submitted Apr 24, 2026).
...
\paragraph{Runs.}
\begin{itemize}
  \item \textbf{V42}: Naive merge of V1 $\to$ 6 layers, fine-tuned 5k steps (lr=2e-4).
  \item \textbf{V43}: Procrustes-aligned merge of V1 $\to$ 6 layers, fine-tuned 5k steps.
\end{itemize}
The merge step runs on a single GPU (job 37579 / 37583 respectively) and auto-submits
the 4-GPU fine-tune job on completion.

\paragraph{Baseline.}
For comparison we also record the zero-shot (pre-fine-tune) PPL of the merged models to
quantify how much is recovered by fine-tuning.  The 12-layer V1 achieves PPL=47.7;
a 6-layer EGPT trained from scratch should fall between V1 and a 6-layer GPT.

\paragraph{Results.}
\emph{V42 and V43 merge+fine-tune jobs submitted Apr 24, 2026 (jobs 37579, 37583).
Results will be added once training completes.}
...
\paragraph{Results.}
\emph{V41 sandwich training submitted Apr 24, 2026 (job 37578, 30k steps).  Layer
cosine-similarity analysis and merger experiments on V41 will be added once training
completes.}

\subsection{Summary Table (to be completed)}

\begin{table}[h]
\centering
\caption{Layer merger experiment overview.  PPL$_0$ = zero-shot post-merge PPL;
  PPL$_{\mathrm{ft}}$ = PPL after 5k fine-tuning steps.
  \emph{All entries pending — jobs running.}}
\label{tab:layer_merger}
\begin{tabular}{lrrrrl}
\toprule
\textbf{Model} & \textbf{Layers} & \textbf{Params} & \textbf{PPL$_0$} & \textbf{PPL$_{\mathrm{ft}}$} & \textbf{Notes} \\
\midrule
...
\begin{figure}[h]
  ...
  \caption{Accuracy vs.\ parameters — all runs including MixH$^\dagger$ (grey).
