...
because the cosine-ramp regulariser requires capturing and comparing
layer-wise activations during the backward pass.
30k training steps thus take ${\approx}$5.3~h wall-time for V53 vs 2.6~h for V1;
at the 4-hour preemptable wall limit this means two job chunks for V53 vs one for V1.
V3 (0.45~s/step, 3.8~h for 30k) fits in a single 4-h chunk.
V4 (${\approx}$0.74~s/step estimated from FLOPs ratio, ${\approx}$6.2~h for 30k)
required multiple 4-h job restarts; its wandb training-length logs
should not be trusted for total step counts due to these preemption interruptions.
At inference time there is no overhead: V53 runs at exactly V1's FLOPs.
