#!/bin/bash
# V0: Standard GPT baseline, d=768, 12L, SwiGLU, RoPE — same setup as V1/V2 EGPT
# ~162M params | 30k steps | 4 GPUs | 7.86B tokens | est. ~3h per job chunk
# Preemptable queue with auto-resume + eval on completion.
#
# Usage: bash run_v0_gpt_baseline.sh

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v0_gpt_baseline_d768.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v0_gpt_baseline_d768
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v0_gpt_baseline.sh
JOB_NAME=gpt_v0_baseline
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v0_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v0_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v0_resume_\${LSB_JOBID}.yml"
