#!/bin/bash
# V10: Mixed-head attention 12×1 — 6 energy heads (V=K) + 6 GPT heads per block
# d=768, int=1536, 12L, standard GPT residual, 144M total (iso-param to V1 143M)
# muP LR=2e-3 (d=768 baseline), 30k steps, 7.86B tokens

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v10_mixed_12x1_d768_lr2e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v10_mixed_12x1_d768_lr2e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v10_mixed_12x1_d768_lr2e3.sh
JOB_NAME=egpt_v10_mixed12x1
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v10_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v10_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v10_resume_\${LSB_JOBID}.yml"
