#!/bin/bash
# V11: Mixed-head attention 6×2 — 6 energy heads (V=K) + 6 GPT heads per block
# d=768, int=2048, 6L, standard GPT residual, 118M total (iso-param to V1 143M)
# muP LR=2e-3 (d=768 baseline), 30k steps, 7.86B tokens

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v11_mixed_6x2_d768_lr2e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v11_mixed_6x2_d768_lr2e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v11_mixed_6x2_d768_lr2e3.sh
JOB_NAME=egpt_v11_mixed6x2
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v11_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v11_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v11_resume_\${LSB_JOBID}.yml"
