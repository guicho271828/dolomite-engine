#!/bin/bash
# V13: V13: Mixed-head attention 8E+4G (12×1) — 8 energy heads (V=K) + 4 GPT heads per block

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=/proj/dmfexp/nima/Code/dolomite-engine/configs/multi_block_ablation/v13_mixed_8e4g_12x1_d768_lr2e3.yml
SAVE_PATH=/proj/dmfexp/nima/Code/dolomite-engine/experiments/energy-inference/results/multi-block-ablation/v13_mixed_8e4g_12x1_d768_lr2e3
SCRIPT_PATH=/proj/dmfexp/nima/Code/dolomite-engine/experiments/energy-inference/scripts/multi-block-ablation/run_v13_mixed_8e4g_12x1_d768_lr2e3.sh
JOB_NAME=egpt_v13_8e4g
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v13_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v13_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v13_resume_\${LSB_JOBID}.yml"
