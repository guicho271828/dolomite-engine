#!/bin/bash
# V15: V15: Energy-gradient mixed heads (12×1) — energy heads use W_Q^T @ softmax @ K (true energy gradient)

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=/proj/dmfexp/nima/Code/dolomite-engine/configs/multi_block_ablation/v15_energy_grad_mixed_12x1_d768_lr2e3.yml
SAVE_PATH=/proj/dmfexp/nima/Code/dolomite-engine/experiments/energy-inference/results/multi-block-ablation/v15_energy_grad_mixed_12x1_d768_lr2e3
SCRIPT_PATH=/proj/dmfexp/nima/Code/dolomite-engine/experiments/energy-inference/scripts/multi-block-ablation/run_v15_energy_grad_mixed_12x1_d768_lr2e3.sh
JOB_NAME=egpt_v15_egrad
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v15_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v15_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v15_resume_\${LSB_JOBID}.yml"
