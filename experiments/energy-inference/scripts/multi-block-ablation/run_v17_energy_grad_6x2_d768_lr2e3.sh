#!/bin/bash
# V17: Energy-gradient mixed heads (6×2, d=768) — recurrent counterpart to V15

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v17_energy_grad_6x2_d768_lr2e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v17_energy_grad_6x2_d768_lr2e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v17_energy_grad_6x2_d768_lr2e3.sh
JOB_NAME=egpt_v17_egrad6x2
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v17_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v17_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v17_resume_\${LSB_JOBID}.yml"
