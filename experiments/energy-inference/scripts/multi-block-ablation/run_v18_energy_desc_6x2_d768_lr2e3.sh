#!/bin/bash
# V18: Energy-descent mixed heads (6×2, d=768) — recurrent counterpart to V16

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v18_energy_desc_6x2_d768_lr2e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v18_energy_desc_6x2_d768_lr2e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v18_energy_desc_6x2_d768_lr2e3.sh
JOB_NAME=egpt_v18_edesc6x2
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v18_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v18_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v18_resume_\${LSB_JOBID}.yml"
