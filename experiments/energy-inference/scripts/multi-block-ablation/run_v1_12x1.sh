#!/bin/bash
# V1: 12 distinct EGPT blocks, dual unconstrained proj, d=768
...
CONFIG=${REPO}/configs/multi_block_ablation/v1_12x1_dual_unconstrained_d768.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v1_12x1_d768
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v1_12x1.sh
JOB_NAME=egpt_v1_12x1
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v1_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v1_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v1_resume_\${LSB_JOBID}.yml"
