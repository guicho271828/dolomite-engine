#!/bin/bash
# V20: Energy-descent mixed heads (~330M, 24×1, d=1024) — scale-up of V16

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v20_energy_desc_24x1_d1024_lr1e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v20_energy_desc_24x1_d1024_lr1e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v20_energy_desc_24x1_d1024_lr1e3.sh
JOB_NAME=egpt_v20_edesc400m
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v20_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v20_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v20_resume_\${LSB_JOBID}.yml"
