#!/bin/bash
# V22: Energy-descent mixed heads (~342M, 6×2, d=1024) — recurrent counterpart to V20

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v22_energy_desc_6x2_d1024_lr1e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v22_energy_desc_6x2_d1024_lr1e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v22_energy_desc_6x2_d1024_lr1e3.sh
JOB_NAME=egpt_v22_edesc400m6x2
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v22_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v22_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v22_resume_\${LSB_JOBID}.yml"
