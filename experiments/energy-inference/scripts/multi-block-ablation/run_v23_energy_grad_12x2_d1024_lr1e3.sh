#!/bin/bash
# V23: Energy-gradient mixed heads (~228M, 12×2, d=1024) — recurrent counterpart to V19

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v23_energy_grad_12x2_d1024_lr1e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v23_energy_grad_12x2_d1024_lr1e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v23_energy_grad_12x2_d1024_lr1e3.sh
JOB_NAME=egpt_v23_egrad400m12x2
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v23_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v23_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v23_resume_\${LSB_JOBID}.yml"
