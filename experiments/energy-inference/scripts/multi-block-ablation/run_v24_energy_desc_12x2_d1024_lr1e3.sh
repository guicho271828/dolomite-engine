#!/bin/bash
# V24: Energy-descent mixed heads (~228M, 12×2, d=1024) — recurrent counterpart to V20

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v24_energy_desc_12x2_d1024_lr1e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v24_energy_desc_12x2_d1024_lr1e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v24_energy_desc_12x2_d1024_lr1e3.sh
JOB_NAME=egpt_v24_edesc400m12x2
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v24_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v24_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v24_resume_\${LSB_JOBID}.yml"
