#!/bin/bash
# V25: Energy-gradient mixed heads (~162M, 6×4, d=1024) — same params as V21 but 4 iters

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v25_energy_grad_6x4_d1024_lr1e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v25_energy_grad_6x4_d1024_lr1e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v25_energy_grad_6x4_d1024_lr1e3.sh
JOB_NAME=egpt_v25_egrad400m6x4
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v25_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v25_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v25_resume_\${LSB_JOBID}.yml"
