#!/bin/bash
# V26: Energy-gradient mixed heads (~162M, 6-layer iter ramp [2,2,3,4,4,9], d=1024)

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v26_energy_grad_6x_ramp_d1024_lr1e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v26_energy_grad_6x_ramp_d1024_lr1e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v26_energy_grad_6x_ramp_d1024_lr1e3.sh
JOB_NAME=egpt_v26_egrad400m6xramp
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v26_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v26_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v26_resume_\${LSB_JOBID}.yml"
