#!/bin/bash
# V2: 6 EGPT blocks × 2 iters each, dual unconstrained proj, d=768
# ~176M params | 30k steps | 4 GPUs | 7.86B tokens | est. ~3-4h per job chunk
# Preemptable queue with auto-resume.
#
# Usage: bash run_v2_6x2.sh

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v2_6x2_dual_unconstrained_d768.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v2_6x2_d768
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v2_6x2.sh
JOB_NAME=egpt_v2_6x2
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v2_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v2_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v2_resume_\${LSB_JOBID}.yml"
