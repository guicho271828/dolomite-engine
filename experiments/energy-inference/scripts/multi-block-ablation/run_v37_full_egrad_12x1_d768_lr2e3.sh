#!/bin/bash
# V37: Full EGrad 12×1 d=768 — EGrad(attn) + Mixed_Energy_MLP (∂E_attn + ∂E_FF)
set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v37_full_egrad_12x1_d768_lr2e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v37_full_egrad_12x1_d768_lr2e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v37_full_egrad_12x1_d768_lr2e3.sh
JOB_NAME=egpt_v37_full_egrad_d768
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v37_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v37_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v37_resume_\${LSB_JOBID}.yml"
