#!/bin/bash
# V38: Full EGrad 24×1 d=1024 — EGrad(attn) + Mixed_Energy_MLP (∂E_attn + ∂E_FF)
set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v38_full_egrad_24x1_d1024_lr1e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v38_full_egrad_24x1_d1024_lr1e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v38_full_egrad_24x1_d1024_lr1e3.sh
JOB_NAME=egpt_v38_full_egrad_d1024
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v38_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v38_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v38_resume_\${LSB_JOBID}.yml"
