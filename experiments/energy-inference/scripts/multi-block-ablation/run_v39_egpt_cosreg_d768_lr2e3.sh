#!/bin/bash
# V39: EGPT 12×1 d=768 + cosine-similarity regulariser (λ=0.01, every 10 steps)
set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v39_egpt_cosreg_12x1_d768_lr2e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v39_egpt_cosreg_12x1_d768_lr2e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v39_egpt_cosreg_d768_lr2e3.sh
JOB_NAME=egpt_v39_cosreg_d768
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v39_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v39_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v39_resume_\${LSB_JOBID}.yml"
