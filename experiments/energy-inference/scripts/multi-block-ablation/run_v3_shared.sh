#!/bin/bash
# V3: Shared backbone (attn+FFN), independent proj per block, d=1152
# ~163M params | 30k steps | 4 GPUs | 7.86B tokens | est. ~4-5h per job chunk
# Requires shared_backbone: true support in CommonConfig + BaseModelMixin
# Preemptable queue with auto-resume.
#
# Usage: bash run_v3_shared.sh

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v3_shared_backbone_d1152.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v3_shared_d1152
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v3_shared.sh
JOB_NAME=egpt_v3_shared
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v3_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v3_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v3_resume_\${LSB_JOBID}.yml"
