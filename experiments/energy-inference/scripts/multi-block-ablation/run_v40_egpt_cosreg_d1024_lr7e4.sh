#!/bin/bash
# V40: EGPT 24×1 d=1024 (~400M) + cosine-similarity regulariser (λ=0.01, every 10 steps)
set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v40_egpt_cosreg_24x1_d1024_lr7e4.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v40_egpt_cosreg_24x1_d1024_lr7e4
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v40_egpt_cosreg_d1024_lr7e4.sh
JOB_NAME=egpt_v40_cosreg_d1024
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v40_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v40_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v40_resume_\${LSB_JOBID}.yml"
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    if [ "\${LATEST_ITER}" -lt "\${NUM_TRAINING_STEPS}" ]; then
        ...
        if [ "\${ALREADY}" -gt 0 ]; then
            echo "Job \${JOB_NAME} already queued (\${ALREADY}). Skipping resubmit."
        else
            echo "Training incomplete (\${LATEST_ITER}/\${NUM_TRAINING_STEPS}). Resubmitting..."
            bash "\${SCRIPT_PATH}"
        fi
    else
