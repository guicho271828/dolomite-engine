#!/bin/bash
# V41: Sandwich model — 2 GPT + 8 EGPT + 2 GPT, d=768 (~160M)
set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v41_sandwich_2gpt8e2gpt_d768_lr2e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v41_sandwich_2gpt8e2gpt_d768_lr2e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v41_sandwich_d768_lr2e3.sh
JOB_NAME=egpt_v41_sandwich_d768
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v41_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v41_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v41_resume_\${LSB_JOBID}.yml"
