#!/bin/bash
# V4: Shared backbone, 4-head wide attention (head_dim=288) + 2x FFN, d=1152
# ~174M params | 30k steps | 4 GPUs | 7.86B tokens | est. ~4-6h per job chunk
# Preemptable queue with auto-resume.
#
# Usage: bash run_v4_shared_wide.sh

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v4_shared_backbone_wide_d1152.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v4_shared_wide_d1152
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v4_shared_wide.sh
JOB_NAME=egpt_v4_wide
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v4_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v4_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v4_resume_\${LSB_JOBID}.yml"
