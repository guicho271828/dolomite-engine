#!/bin/bash
# V54: Parallel GPT 160M baseline, d=768, 12L, softmax_attn, standard MLP
# h = h + proj_attn(attn(ln(h))) + proj_mlp(ffn(ln(h)))  [parallel, additive residual]
# ~176M params | 30k steps | 4 GPUs | 7.86B tokens | est. ~3h per job chunk
# Preemptable queue with auto-resume + eval on completion.

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v54_parallel_gpt_12x1_d768_lr2e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v54_parallel_gpt_12x1_d768_lr2e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v54_parallel_gpt_d768.sh
JOB_NAME=egpt_v54_pgpt160m
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v54_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v54_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v54_resume_\${LSB_JOBID}.yml"
