#!/bin/bash
# V55: Parallel GPT 400M baseline, d=1024, 24L, softmax_attn, standard MLP
# h = h + proj_attn(attn(ln(h))) + proj_mlp(ffn(ln(h)))  [parallel, additive residual]
# ~405M params | 30k steps | 4 GPUs | 7.86B tokens | est. ~3h per job chunk
# Preemptable queue with auto-resume + eval on completion.

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v55_parallel_gpt_24x1_d1024_lr1p5e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v55_parallel_gpt_24x1_d1024_lr1p5e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v55_parallel_gpt_d1024.sh
JOB_NAME=egpt_v55_pgpt400m
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v55_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v55_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v55_resume_\${LSB_JOBID}.yml"
