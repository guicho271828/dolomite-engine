#!/bin/bash
# V6: helmholtz_factored — dx = -M(attn + A*ffn), M=PSD, A=antisym
# User formulation: dx = -proj_psd(attn + proj_anti(ffn))
# ~140M params | 30k steps | 8 GPUs | 7.86B tokens | est. ~1.5-2h per job chunk
# Preemptable queue with auto-resume.

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v6_12x1_helmholtz_factored_d768_lr2e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v6_12x1_d768_helmholtz_factored_lr2e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v6_helmholtz_factored.sh
JOB_NAME=egpt_v6_hlmholtz_f
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v6_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v6_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v6_resume_\${LSB_JOBID}.yml"
