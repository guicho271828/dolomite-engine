#!/bin/bash
# V7: helmholtz_dual — dx = -(R*attn + A*ffn), R=PSD (gradient), A=antisym (curl)
# Clean Helmholtz split: attn=gradient/descent, FFN=curl/rotation
# ~140M params | 30k steps | 8 GPUs | 7.86B tokens | est. ~1.5-2h per job chunk
# Preemptable queue with auto-resume.

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v7_12x1_helmholtz_dual_d768_lr2e3.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v7_12x1_d768_helmholtz_dual_lr2e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v7_helmholtz_dual.sh
JOB_NAME=egpt_v7_hlmholtz_d
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v7_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v7_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v7_resume_\${LSB_JOBID}.yml"
