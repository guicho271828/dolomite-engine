#!/bin/bash
# V9: GPT 400M iso-param baseline for comparison with V1 400M EGPT
# d=1024, 24L, int=2048, 3-matrix SwiGLU, softmax attention — 354M params
# Same token budget as V1 400M: 30k × 4 GPUs × batch=4 × grad_accum=4 × seq=4096 = 7.86B tokens
# muP LR: 2e-3 × 768/1024 ≈ 1.5e-3
# Est. ~1.5h per 4h chunk on 4 A100s (GPT faster than EGPT — no proj layers)

set -euo pipefail
REPO=/proj/dmfexp/nima/Code/dolomite-engine
CONFIG=${REPO}/configs/multi_block_ablation/v9_gpt_baseline_d1024.yml
SAVE_PATH=${REPO}/experiments/energy-inference/results/multi-block-ablation/v9_gpt_baseline_d1024_lr1e3
SCRIPT_PATH=${REPO}/experiments/energy-inference/scripts/multi-block-ablation/run_v9_gpt_baseline_d1024.sh
JOB_NAME=egpt_v9_gpt400m
...
if [ -f "\${LATEST_JSON}" ]; then
    ...
    TMPCONFIG="/tmp/v9_resume_\${LSB_JOBID}.yml"
    cp "\${CONFIG}" "\${TMPCONFIG}"
...
[ -f "/tmp/v9_resume_\${LSB_JOBID}.yml" ] && rm -f "/tmp/v9_resume_\${LSB_JOBID}.yml"
